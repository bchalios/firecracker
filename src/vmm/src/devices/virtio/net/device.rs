...
impl Net {
    ...
    fn rate_limited_rx_single_frame(&mut self, bytes: usize) -> bool {
        let rx_queue = &mut self.queues[RX_INDEX];
        if !Self::rate_limiter_consume_op(&mut self.rx_rate_limiter, bytes as u64) {
        ...
        loop {
            match self.read_from_mmds_or_tap() {
                ...
                Ok(count) => {
                    ...
                    self.metrics.rx_bytes_count.add(count as u64);
                    self.metrics.rx_packets_count.inc();
                    if !self.rate_limited_rx_single_frame(count) {
                        break;
                    }
                }
                Err(NetError::IO(err)) => {
    ...
    fn process_tx(&mut self) -> Result<(), DeviceError> {
        ...
        let deferred_frame = self.has_deferred_frame();
        ...
        while let Some(head) = tx_queue.pop_or_enable_notification() {
            ...
            .unwrap_or(false);
            if frame_consumed_by_mmds && !deferred_frame {
...
pub mod tests {
    ...
    fn test_rx_retry() {
        ...
        assert!(!th.net().has_deferred_frame());
