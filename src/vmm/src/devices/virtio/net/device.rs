...
impl RxBuffers {
    ...
    ///
    /// The `size` should be the number of bytes previously written in the [`RxBuffers`]. This
    /// means that `size` cannot be more than the currently available bytes in the underlying
    /// `IoVecBufferMut`.
    fn mark_used(&mut self, size: u32, metrics: &NetDeviceMetrics) {
        ...
        // We are trying to mark as used more bytes than a single descriptor. However without
        // VIRTIO_NET_F_MRG_RXBUF negotiated (we don't support it currently) the guest can't handle
        // frames spanning multiple DescriptorChains. There is nothing we can do now, just pass the
        // descriptor back to the guest without claiming any written bytes
        if size > parsed_dc.length {
            parsed_dc.length = 0;
            metrics.rx_fails.inc();
        } else {
            parsed_dc.length = size;
            metrics.rx_bytes_count.add(size as u64);
            metrics.rx_packets_count.inc();
        }
        self.header_set_num_buffers(1);
        self.iovec.drop_descriptor_chain(&parsed_dc);
        self.deferred_descriptor = Some(parsed_dc);
    }
    ...
    fn finish_frame(&mut self, rx_queue: &mut Queue) -> bool {
        match self.deferred_descriptor.take() {
            Some(used_dc) => {
                if let Err(err) = rx_queue.add_used(used_dc.head_index, used_dc.length) {
                    error!(
                        "net: Failed to add used descriptor {} of length {} to RX queue: {err}",
                        used_dc.head_index, used_dc.length
                    );
                    false
                } else {
                    true
...
impl Net {
    ...
    fn rate_limited_rx_single_frame(&mut self) -> bool {
        ...
        if !self.rx_buffer.finish_frame(rx_queue) {
    ...
    fn read_from_mmds_or_tap(&mut self) -> Result<usize, NetError> {
        ...
        if let Some(ns) = self.mmds_ns.as_mut() {
                ...
                ns.write_next_frame(frame_bytes_from_buf_mut(&mut self.rx_frame_buf)?)
            {
                ...
                self.rx_buffer
                    .mark_used((vnet_hdr_len() + len).try_into().unwrap(), &self.metrics);
                return Ok(vnet_hdr_len() + len);
            }
        }

        let len = self.read_tap().map_err(NetError::IO)?;
        self.rx_buffer
            .mark_used(len.try_into().unwrap(), &self.metrics);
        Ok(len)
    }
    ...
    fn handle_deferred_frame(&mut self) -> Result<(), DeviceError> {
        if self.rate_limited_rx_single_frame() {
            ...
            self.rx_buffer.deferred_descriptor = None;
    ...
    pub(crate) fn read_tap(&mut self) -> std::io::Result<usize> {
