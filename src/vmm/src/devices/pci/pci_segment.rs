...
impl std::fmt::Debug for PciSegment {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PciSegment")
            ...
            .field("mem32_allocator", &self.mem32_allocator)
            .field("mem64_allocator", &self.mem64_allocator)
            .finish()
    }
}
...
impl PciSegment {
    ...
    pub(crate) fn new(
        ...
        info!(
            "pci: adding PCI segment: id={}, PCI MMIO config address: 0x{:x}, mem32 area: [0x{:x}-0x{:x}], mem64 area: [0x{:x}-0x{:x}] IO area: [{PCI_CONFIG_IO_PORT:#x}-{:#x}]",
            segment.id,
    ...
    pub(crate) fn new(
        ...
        info!(
            "pci: adding PCI segment: id={}, PCI MMIO config address: 0x{:x}, mem32 area: [0x{:x}-0x{:x}], mem64 area: [0x{:x}-0x{:x}]",
            segment.id,
            segment.mmio_config_address,
            segment.start_of_mem32_area,
            segment.end_of_mem32_area,
    ...
    pub fn reserve_legacy_interrupts_for_pci_devices(
        ...
        let num_irqs = 8;
        let irqs = resource_allocator.allocate_gsi(num_irqs)?;

        // There are 32 devices on the PCI bus, let's assign them an IRQ.
        for i in 0..32 {
            pci_irq_slots[i] = irqs[i % num_irqs as usize].try_into().unwrap();
        }
